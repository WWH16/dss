...
{{-- CHANGED (whole file): every rounded-lg became rounded-md (cards stay rounded-xl); font-black became font-bold; field labels, the portal tag and the password hint went from neutral-400 to neutral-500 (the hint also 10px to 11px); the "Update Password" button went from neutral-900 to brand-600, like "Save Changes" in the other modal. --}}
        ...
        <div class="p-6 sm:p-8 border-b border-neutral-100 bg-neutral-50/50 flex flex-col sm:flex-row items-center sm:items-start justify-between gap-6 relative">
            ...
            <div class="flex flex-row sm:flex-col gap-2 shrink-0 w-full sm:w-auto">
                <button type="button" onclick="openEditProfileModal()" 
                    class="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3.5 py-2 bg-brand-600 hover:bg-brand-700 text-white text-xs font-bold rounded-md transition-colors shadow-2xs">
                    <ion-icon name="create-outline" class="text-sm"></ion-icon>
                    Edit Profile
                </button>
                <button type="button" onclick="openChangePasswordModal()" 
                    class="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3.5 py-2 bg-white hover:bg-neutral-50 text-neutral-700 border border-neutral-200/90 text-xs font-bold rounded-md transition-colors shadow-2xs">
...
<dialog id="edit-profile-modal" class="confirm-modal modal-sharp max-w-md w-full rounded-xl p-0 overflow-hidden shadow-2xl border-0 outline-none bg-white backdrop:bg-neutral-950/60">
    <div class="bg-white">
        ...
        <form action="{{ route('student.profile.update') }}" method="POST" class="p-6 space-y-4">
            ...
            {{-- Course / Degree --}}
            {{-- CHANGED: the placeholder suggests program codes only. The old example "BS Computer Science" does not match any code the admin Students page groups into a department, so a student who typed it was listed under no department. --}}
            <div>
                <label for="modal-course" class="block text-xs font-bold text-neutral-700 mb-1.5">Course / Degree Program</label>
                <input type="text" id="modal-course" name="course" value="{{ old('course', $profile->course) }}" placeholder="e.g. BSIT, BSCS, BSHM"
                    class="w-full px-3 py-2 bg-neutral-50 border @error('course') border-rose-500 @else border-neutral-200 @enderror rounded-md text-xs font-medium text-neutral-900 focus:outline-none focus:border-brand-700 focus:bg-white transition-colors">
                @error('course')
                    <p class="text-rose-600 text-[11px] font-semibold mt-1">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="bg-white">
        ...
        <form action="{{ route('student.profile.password') }}" method="POST" class="p-6 space-y-4">
            ...
            <div>
                ...
                <p class="text-[11px] text-neutral-500 mt-1">Minimum 8 characters with letters, numbers, and symbols.</p>
                @error('password')
                    <p class="text-rose-600 text-[11px] font-semibold mt-1">{{ $message }}</p>
                @enderror
            </div>
